
import java.util.Scanner;


public class GameMain {

    public static void main(String[] args) {

        /*
         * ABCDE_rpg 게임
         * 1. 프로그램이 실행되면 메뉴를 출력한다.
         * 2. 메뉴는 (1.캐릭터 생성, 2.캐릭터 정보, 3.전직하기, 4.공격하기, 0.종료)
         *    - (1.캐릭터 생성)을 선택한 경우, 이름과 스탯값을 초기값으로 하는 Novice(평민)클래스형 인스턴스를 생성한다.
         *      스탯은 (힘, 민첩, 지능)이 있으며 각각 랜덤한 값을 가지고,
         *      형평성을 위해 스탯의 총 합이 15이상인 경우에만 저장되도록 하며,
         *      사용자의 선택에 따라 해당 스탯을 저장 or 스탯 재부여 할 수 있도록 한다.
         * 3. - (2.캐릭터 정보)를 선택한 경우, 생성된 캐릭터의 이름, 직업, 힘, 민첩, 지능을 출력한다.
         * 4. - (3.전직하기)를 선택한 경우, 생성된 캐릭터의 정보를 초기값으로 가지는
         *      선택한 클래스에 해당하는 인스턴스를 생성하고 문구를 출력한다.
         * 5. - (4.공격하기)를 선택한 경우, Attack()메서드를 호출하여 직업(인스턴스)에 따라 공격한다.
         * 6. - (5.스킬사용)을 선택한 경우, Skill()메서드를 호출하여 직업(인스턴스)에 따라 스킬을 사용한다.
         * 7. - (0.종료)를 선택한 경우, 프로그램을 종료한다.
         * 8. - 종료를 선택하기 전까지 반복한다.
         * */

        Scanner scan = new Scanner(System.in);

        GameMain_Method gameMain_method = new GameMain_Method();

        Novice character = null;
        Skill skill = null;
        int num = 0;

        while(true) {

            gameMain_method.printMenu();

            num = scan.nextInt();

            switch (num) {
                case 1:
                    character = gameMain_method.createCharacter();
                    break;
                case 2:
                    if(character != null) {
                        gameMain_method.characterInfo(character);
                    }
                    else{
                        gameMain_method.printText(3);
                    }
                    break;
                case 3:

                    if(character != null){
                        character = gameMain_method.jobSelect(num, character);
                    }
                    else{
                        gameMain_method.printText(3);
                    }

                    break;
                case 4:

                    if(character != null){
                        character.attack();
                    }
                    else{
                        gameMain_method.printText(3);
                    }
                    break;
                case 5:

                    if(character != null) {
                        skill = gameMain_method.skill(character);
                    }
                    else{
                        gameMain_method.printText(3);
                }
                    break;
                case 0:
                    gameMain_method.printText(1);
                    return;
                default:
                    gameMain_method.printText(2);
                    break;
            }
        }
    }
}
